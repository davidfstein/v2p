...
class Meka(MLClassifierBase):
    ...
    def fit(self, X, y):
        ...
        self.train_data_ = self.convert(X, y)
        train_arff = 'train.arff'# tempfile.NamedTemporaryFile(delete=False)
        classifier_dump_file = 'clf.dmp'#tempfile.NamedTemporaryFile(delete=False)

        try:
            with open(train_arff, "w") as fp:
            ...
            input_args = [
                ...
                '"{}"'.format(train_arff),
                "-d",
                '"{}"'.format(classifier_dump_file),
            ]

            self._run_meka_command(input_args)
            self.classifier_dump = None
            with open(classifier_dump_file, "rb") as fp:
                self.classifier_dump = fp.read()
        finally:
            pass
            # self._remove_temporary_files([train_arff])

        return self

    def convert(self, X, y, filename=None):
        X = X.copy()
    ...
    def predict(self, X):
        ...
        y = np.zeros((self._instance_count, self._label_count), dtype=int)

        try:
            train_arff = "train.arff"#tempfile.NamedTemporaryFile(delete=False)
            test_arff = "test.arff"#tempfile.NamedTemporaryFile(delete=False)
            classifier_dump_file = "clf.dmp"#tempfile.NamedTemporaryFile(delete=False)

            with open(train_arff, "w") as fp:
                fp.write(self.train_data_)

            # with open(classifier_dump_file, "wb") as fp:
            #     fp.write(self.classifier_dump)

            with open(test_arff, "w") as fp:
                fp.write(self.convert(X, y))

            args = ["-l", '"{}"'.format(classifier_dump_file)]
            ...
            pass
            # self._remove_temporary_files([train_arff, test_arff])
    ...
    def _parse_output(self):
        ...
        joblib.dump(self.output_, 'preds.joblib')
        ...
        predictions = [[float(a) for a in z[0].split(' ')] for z in predictions]
        # print(np.array(predictions), np.array(predictions).shape)
        self._results = predictions

        # self._results = np.zeros(
        #     (self._instance_count, self._label_count), dtype="float"
        # )
